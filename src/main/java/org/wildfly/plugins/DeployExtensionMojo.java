...
public class DeployExtensionMojo extends AbstractMojo {
	 ...
	 * to {@link #modulesHome} directory.
	 */
	@Parameter()
	private File moduleZip;
	
	/**
	 * An alternative to {@link #moduleZip} parameter. This parameter has higher priority than {@link #moduleZip}.
	 * A string of the form groupId:artifactId:version[:packaging][:classifier]. Default for packaging is <strong>zip</strong> and for classifier it's <strong>module</strong>
	 * @since 0.7
	 */
	@Parameter()
	...
	/**
	 * Location of modules home (either relative to {@link #jbossHome} or absolute). Set this value unless your structure inside {@link #moduleZip} does not include path to modules.
	...
	/**
	 * Location where plugin will backup original server configuration file (standalone.xml) - can be either relative to {@link #jbossHome} or absolute
	 */
	...
	/**
	 * List of data to be inserted to {@link #serverConfig}. This is pretty powerful stuff to put/replace any XML content
	 * anywhere in {@link #serverConfig} 
	 */
	@Parameter
	private Insert[] edit;
	...
	private File resolveArtifactModuleZip() throws MojoExecutionException {
		...
		// we now how output file will look like because we call maven-dependency-plugin with stripClassifier=true and stripVersion=true
